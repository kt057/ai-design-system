...
`@kt057/ai-design-system` is a small, opinionated React design system built around three pillars:
...
- ❌ **Not styled with CSS-in-JS.** All styling is Tailwind utility classes generated through CVA. The published `dist/styles.css` is self-contained (tokens + utility classes), so consumers just `import "@kt057/ai-design-system/styles.css"` once and render — no Tailwind config required on their side.
...
```ts
import { Button, type ButtonProps, cn } from "@kt057/ai-design-system";
...
- **`rules/`** — this directory. Imported via `@kt057/ai-design-system/rules/<file>` if needed.
